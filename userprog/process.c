...
process_wait (tid_t child_tid) {
  ...
  if (!list_empty (child_list)) {
    ...
    while (cur != list_tail (child_list)) {
      ...
      /* 자식을 찾은 뒤, 필요한 정보를 꺼내고, child_list에서 제거하고,
       * child_list_elem을 free한다. */
      if (target->child_tid == child_tid) {
...
process_exit (void) {
  ...
  while (!list_empty (&curr->child_list)) {
    struct child_list_elem *tgt = list_entry (
        list_pop_front (&curr->child_list), struct child_list_elem, elem);
